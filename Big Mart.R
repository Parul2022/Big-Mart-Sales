...
train <- read.csv("./Big Mart/Train.csv", na.strings=c("","NA"))
test <- read.csv("./Big Mart/Test.csv", na.strings=c("","NA"))
...
#From the above graph we can see that there are variations in Item_MRP with Item_Type
...
#Imputing mean in place of missing values in Item_weight
...
#Imputing mean in place of missing values in Item_visibility
...
#Imputing for missing value in Outlet_size by randomization
set.seed(125)
x <- sample(levels(complete[[9]]),sum(is.na(complete[[9]])), replace = TRUE)
complete[[9]][is.na(complete[[9]])]<- x

#Modifying levels of Item_Fat_Content
...
#Converting Outlet_Establishment_Year from categorical to numeric by substituting no. of years since establishment of outlet for Outlet_Establishment_Year
h <- as.numeric(as.character(complete$Outlet_Establishment_Year))
complete$Outlet_Estd_YearPassed <- 2013 - h

#Creating new features Outlet_Count and Item_Count to replace categorical Item_Identifier and Outlet_Identifier
...
#Modifying levels of Item_Type based on Item_Identifier
...
#Conversion of categorical or factor variables to numeric using One hot encoding 
...
#Dropping Item_Fat_Content, Outlet_Location_Type, Item_count
...
mod_train3 <- select(.data = train, Item_Count, Outlet_Count,train$Item_MRP,Item_Visibility, Outlet_Estd_YearPassed, Item_Outlet_Sales)
...
# Fitting model deleting all outliers 
...
#Fitting model with just outliers
...
#we see that adjusted R squared incresases on fitting a model deleting outliers but that is not a statistically significant method.
...
new_test <- test[ ,1:16]
new_train <- train[ ,c(-1,-14,-8,-16)]
new_test <- test[ ,c(-1,-14,-8,-16)]
